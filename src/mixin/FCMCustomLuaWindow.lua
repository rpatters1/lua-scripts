        ...
        if self["Register" .. f .. "_"] then
            ...
            if f == "CloseWindow" then
                self["Register" .. f .. "_"](
                    self, function()
                        cb()
                        self:StorePosition(true)
                        private[self].StoredX = self.StoredX
                        private[self].StoredY = self.StoredY
                        private[self].StoredWidth = self.StoredWidth
                        private[self].StoredHeight = self.StoredHeight
                    end)
            else
...
Disabling this will also disable automatic restoration of window's size.
...
@ enabled (boolean)
    ...
    function props:SetAutoRestorePosition(enabled)
        ...
        if not enabled then
            private[self].AutoRestoreSize = false
        end
    end
...
**[>= v0.60] [Fluid]**
Enables/disables automatic restoration of the window's size on subsequent openings.
Enabling this will also enable automatic restoration of window's position.
Note that restoring the size of a non-user-resizeable window can have unexpected results.
This is disabled by default.

@ self (FCMCustomLuaWindow)
@ enabled (boolean)
]]
    function props:SetAutoRestoreSize(enabled)
        mixin.assert_argument(enabled, "boolean", 2)

        private[self].AutoRestoreSize = enabled

        if enabled then
            private[self].AutoRestorePosition = true
        end
    end
...
**[>= v0.60]**
Returns whether automatic restoration of window size is enabled.

@ self (FCMCustomLuaWindow)
: (boolean) `true` if enabled, `false` if disabled.
]]
    function props:GetAutoRestoreSize()
        return private[self].AutoRestoreSize
    end
...
@ height (number)
    ...
    function props:SetRestorePositionData(x, y, width, height)
        ...
        if self:HasBeenShown() and not self:WindowExists() then
            ...
            private[self].StoredWidth = width
            private[self].StoredHeight = height
        end
    end
